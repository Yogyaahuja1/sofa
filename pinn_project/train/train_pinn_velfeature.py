...
from pinn_model import LagSequenceAttentionAccelExtra

# ============================================================
# LAG-WINDOW SWEEP — same pipeline as train_pinn_seqattn_accel.py, but N_LAGS
# and N_EPOCHS are command-line args so multiple window sizes can be compared
# quickly (shorter budget) before committing to a full training run on the
# winner. Appends one summary line per run to lag_sweep_results.txt.
# ============================================================
...
# Sustained-contact flag (same definition as the contact-weight experiment) —
# kept here purely for reporting a comparable metric, not used in the loss.
df_fmag_sustain = np.linalg.norm(df[['tool_fx', 'tool_fy', 'tool_fz']].values, axis=1)
is_sustained = np.zeros(N_rows, dtype=np.float32)
for sid in df['session_id'].unique():
    if sid == 0:
        continue
    idx = df.index[df['session_id'] == sid].values
    high = df_fmag_sustain[idx] > 3.0
...
# Smoothed (longer-window) velocity magnitude — average |v| over the last
# VEL_SMOOTH_WINDOW rows within the same session, clamped at session start.
# Targets a specific, narrow gap: short-window features can't distinguish "low,
# noisy micro-motion during a held press" from "low velocity because nothing is
# happening" — both look similar over 8 lags, but very different over a longer
# span. Added as ONE extra global scalar, not more per-lag tokens, to avoid the
# sharp-transition accuracy cost that came from extending N_LAGS itself to 12.
VEL_SMOOTH_WINDOW = 15
vel_mag_all = np.sqrt(tool_vx_all**2 + tool_vy_all**2 + tool_vz_all**2)
vel_smooth = np.zeros(N_rows, dtype=np.float32)
for row in range(N_rows):
    lo = max(0, row - VEL_SMOOTH_WINDOW + 1)
    # don't reach across a session boundary
    seg_start = lo
    for i in range(row, lo - 1, -1):
        if session_ids[i] != session_ids[row]:
            seg_start = i + 1
            break
    vel_smooth[row] = vel_mag_all[seg_start:row+1].mean()
corr_vel_smooth, _ = pearsonr(vel_smooth, force_mag_all)
print(f"vel_smooth (window={VEL_SMOOTH_WINDOW}) corr with force_mag:        {corr_vel_smooth:.3f}")
...
X_combined = np.concatenate([
    ...
    accel,                        # (N, 3)   idx  960-962  lag1-vs-lag3 acceleration
    vel_smooth.reshape(-1, 1),     # (N, 1)   idx  963      NEW — longer-window smoothed |v|
], axis=1).astype(np.float32)

N_INPUTS = X_combined.shape[1]
X_raw_tensor = torch.FloatTensor(X_combined.copy())
print(f"Total inputs: {N_INPUTS}")  # should print 964 for n_lags=8 (190*8+13+1)
...
# ============================================================
# STEP 8: TRAIN/TEST SPLIT (random, then shuffle train)
# ============================================================
...
model = LagSequenceAttentionAccelExtra(n_output=N_OUT, n_inputs=N_INPUTS, n_lags=N_LAGS, n_extra=1).to(device)
...
best_epoch    = 0
MODEL_BEST    = f'tissue_pinn_velfeature_n{N_LAGS}_best.pth'
...
torch.save({
    ...
    'n_lags': N_LAGS,
}, f'tissue_pinn_velfeature_n{N_LAGS}.pth')
print(f"Model saved to tissue_pinn_velfeature_n{N_LAGS}.pth")

with open('velfeature_results.txt', 'a') as f:
    f.write(f"n_lags={N_LAGS:3d}  vel_smooth_window={VEL_SMOOTH_WINDOW}  epochs_run={best_epoch:5d}  "
            ...
            f"params={n_params:,}\n")
print("Summary appended to velfeature_results.txt")
